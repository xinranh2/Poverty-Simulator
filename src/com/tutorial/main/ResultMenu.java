...
public class ResultMenu extends GameObject implements KeyListener {
	//these are for the whole menu itself
		...
		public ResultMenu(MenuDataArchive inputArchive) {
			...
			for (int i = 0; i < numChoices; i++) {
				...
				System.out.println(archive.getOptionStrings()[i][0]);
				//String beter = archive.getOptionStrings()[i][0];
			...
			//this.choiceStrings = archive.getOptionStrings()[1]; //the 1 index in the getOptionsStrings array is for all the options.
			//this.choiceIdArray = new String[this.choiceStrings.length];
			//setUpTheMenuStrings(archive.getOptionStrings());
			setReticleY();
		...
		public void setUpTheMenuStrings(String[][] setMenuStrings) {
			for (int i = 0; i < setMenuStrings.length; i++) {
				//this.choiceStrings[0] = setMenuStrings[1][0]; //it gets a null pointer here. we know the right side is okay. left side bad.
